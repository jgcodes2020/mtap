...
namespace details {
template <class T, size_t I> using index_type_sink = T;

template <class ISeq> struct callback_sig_helper;

template <size_t... Is> struct callback_sig_helper<std::index_sequence<Is...>> {
...
constexpr bool callable_helper(std::type_identity<T>,
...
concept callable = callable_helper(std::type_identity<T>{},
                                   std::type_identity<F>{});
...
static constexpr std::optional<opt_type> classify_opt(std::string_view str,
                                                      size_t nargs) {
  ...
  if (cmp < 0 || str[0] != '-')
    ...
    auto test_fn = [](char c) { return c == '-' || isalnum(c); };
...
template <fixed_string Switch, size_t NArgs,
          callable<make_callback_sig<NArgs>> F>
struct opt_impl {
  static_assert(classify_opt(Switch, NArgs).has_value(),
...
template <class T> struct is_some_opt : std::false_type {};
template <fixed_string N, size_t S,
...
namespace details {
template <class ShortSeq = type_sequence<>, class LongSeq = type_sequence<>,
          class PAIndex = void, size_t Index = 0>
struct option_filter;

template <class PA> struct eval_posarg_index;

template <> struct eval_posarg_index<void> {
...
template <fixed_string... SNs, size_t... SIs, fixed_string... LNs,
          size_t... LIs, class PA, size_t I>
struct option_filter<type_sequence<string_index_leaf<SIs, SNs>...>,
  ...
  constexpr auto operator+(std::type_identity<opt_impl<N, S, F>>) {
    if constexpr (opt_impl<N, S, F>::type == opt_type::short_opt) {
      return option_filter<type_sequence<string_index_leaf<SIs, SNs>...,
                                         string_index_leaf<I, N>>,
                           type_sequence<string_index_leaf<LIs, LNs>...>, PA,
                           I + 1>{};
      ...
      static_assert(std::is_void_v<PA>,
...
constexpr size_t count_shorts(type_sequence<opt_impl<Ss, Ns, Fs>...>) {
  ...
  return std::count_if(pairs.begin(), pairs.end(),
                       [](const std::pair<std::string_view, size_t> &pair) {
                         return classify_opt(pair.first, pair.second) ==
                                opt_type::short_opt;
                       });
}
template <fixed_string... Ss, size_t... Ns, class... Fs>
...
template <fixed_string... Ss, size_t... Ns, class... Fs>
constexpr std::array<std::pair<std::string_view, size_t>,
...
template <class... Opts> class parser;

template <fixed_string... Ns, size_t... Ss, class... Fs>
class parser<details::opt_impl<Ns, Ss, Fs>...> {
  static_assert(string_pack_unique_v<Ns...>,
                "All option switches must be unique");

private:
  using vtable_short_t =
      std::unordered_map<char, size_t (*)(std::tuple<Fs...> &, int,
                                          const char *[], int, int)>;
  using vtable_long_t =
      std::unordered_map<std::string_view, size_t (*)(std::tuple<Fs...> &, int,
                                                      const char *[], int)>;

  using option_filter_t =
      decltype((details::option_filter<>{} + ... +
  ...
  template <size_t I>
  static size_t dispatch_short(std::tuple<Fs...> &tup, int argc,
                               const char *argv[], int iarg, int clip) {
  ...
  template <size_t I>
  static size_t dispatch_long(std::tuple<Fs...> &tup, int argc,
                              const char *argv[], int iarg) {
    constexpr size_t arg_size =
        integer_sequence_element_v<I, std::index_sequence<Ss...>>;
    if (iarg + arg_size >= argc)
      throw argument_error("Not enough arguments remaining");
  ...
  static vtable_long_t make_long_vtable() {
    constexpr auto vals = details::filter_longs(
        type_sequence<details::opt_impl<Ns, Ss, Fs>...>{});
    return [&]<size_t... Is>(std::index_sequence<Is...>) {
      return vtable_long_t{{vals[Is].first, dispatch_long<vals[Is].second>}...};
    }
    (std::make_index_sequence<vals.size()>{});
  }
...
public:
  parser(details::opt_impl<Ns, Ss, Fs> &&...opts)
      : ctable(new std::tuple<Fs...>(std::move(opts.fn)...)),
        short_vtable(make_short_vtable()), long_vtable(make_long_vtable()) {}
